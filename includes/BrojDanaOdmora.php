...
class BrojDanaOdmora
{
    public static function izracunaj(Radnik $radnik): int
    {
        ...
        if ($is_neodredjeno) {
            ...
            $broj_dana = 20;
            $iskoriceni_broj_dana_ove_godine = ZahteviZaOdmor::prebroj($radnik->id, ((int) date('Y')));
            $broj_dana = $broj_dana - $iskoriceni_broj_dana_ove_godine;
            ...
            if ((int) date('Y', strtotime($datum_pocetka_rada . ' 03:00:00')) < (int) date('Y')) {
                ...
                $broj_dana = floor(20 / 12 * broj_punih_meseci($datum_pocetka_rada, date('Y-m-d')));
            }
            $iskoriceni_broj_dana_ove_godine = ZahteviZaOdmor::prebroj($radnik->id, ((int) date('Y')));
            $broj_dana = $broj_dana - $iskoriceni_broj_dana_ove_godine;
            return $broj_dana;
